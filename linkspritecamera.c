...
const int jpegstrlen=3500;			//length of jpeg return data, must be an interger multiple of 8
						// and it seemes that  the read buffer in linux is around 4k
unsigned int fd, n;				// file description for the serial port and counter
unsigned char jpegstop=1, lastchar=0;		//end of jpeg marker
unsigned char smallbuffer[65536]="";
...
void ReadImageData()
{
 ...
 unsigned int MM=0, MMH=0, kkh=0, zero=0;
 ...
 kkh=jpegstrlen>>8;
 write(fd,&kkh,1);
 write(fd,&jpegstrlen,1);
 write(fd,&readdatal,2);
 
 usleep(300*jpegstrlen); // delay to get return data

 printf("Trying to read readimagedata data.\n");
 ReadData();

  for(k=5;k<(jpegstrlen+5);k++)					//extract substring from return data
   {
 ...
 printf("MM is %d.\n",MM);
 printf("jpegstop %d.\n\n\n", jpegstop);
 MM+=jpegstrlen;                            		//address increas using return data length
 }
}
...
{
	int index=0;
	n = read(fd,&smallbuffer,(jpegstrlen+20));
	printf("%d  ",n);
